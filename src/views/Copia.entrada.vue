    ...
    <!-- <v-data-table
      ...
      >
        <template v-slot:body="{ items, isSelected, select}">
          <tbody>
            <tr v-for="d in items" :key="d.id_articulo"> 
              <td>
                <v-checkbox
...
export default {
    ...
    computed: {
      ...
      categories () {
        ...
        return this.desserts.filter(item => {
          const text = item.codigo.toLowerCase()
    ...
    methods: {
        ...
        eliminar(index){
            console.log(this.editedItem.articulos[index]);
        ...
        elegir(item){
            console.log(item.chequear);
            if (item.chequear == false) {
            
             
            } else if (item.chequear == true) {
        ...
        registrar(){
          ...
          /*   let perfil = JSON.parse(localStorage.getItem('usuario'))  
            ...
            breakit() {
      this.desserts = JSON.parse(JSON.stringify(this.desserts))
    },
